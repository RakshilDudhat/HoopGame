...
extension GameBoardScene {
    ...
    func loadNextHoop(_ bucketNode: SKSpriteNode, count: Int) {
        let hoopPosition = bucketNode.position
        viewModel.calculatePoints(count, isBankShot: isBankShot, hoopPosition: hoopPosition)
        for _ in 0..<count {
    ...
    func handleMissing() {
        ...
        if viewModel.lives == 0 {
            viewModel.dependency.gameManager.completeGame(mode: viewModel.dependency.gameMode)
    ...
    func reloadNewGame() {
        hoopNodes.removeAll()
        let restartMode = viewModel.dependency.gameMode.restartMode
        viewModel.loadGame(on: frame, mode: restartMode)
        configureBallNode()
        configureHoopNodes()
        startAnimationOnBallNode()
    }
